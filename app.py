...
def analisar_oportunidades(df_calc, mapa_nomes):
    ...
    for ticker in tickers:
        try:
            ...
            # === VOLATILIDADE HISTÓRICA (HV) — será convertida em percentil após o loop ===
            # HV = desvio padrão anualizado dos log-retornos (padrão de mercado)
            # O ranking final 0-10 é calculado por percentil relativo entre todas as BDRs
            try:
                close_s  = df_ticker['Close'].tail(63)  # ~3 meses de pregões
                log_ret  = np.log(close_s / close_s.shift(1)).dropna()
                hv_anual = log_ret.std() * np.sqrt(252) * 100  # % anualizado
                hv_raw   = hv_anual if not np.isnan(hv_anual) else 999
            except Exception:
                hv_raw = 999
    ...
    # === CONVERTER HV_raw PARA RANKING PERCENTIL 0-10 (auto-calibrado) ===
    # Menor HV = mais estável = nota mais alta (inversão)
    if resultados:
        hvs = [r['HV_raw'] for r in resultados]
        hv_min, hv_max = min(hvs), max(hvs)
        amplitude = hv_max - hv_min if hv_max > hv_min else 1
        for r in resultados:
            # Percentil invertido: HV baixa → próximo de 1.0 → nota 10
            percentil_inv = 1 - (r['HV_raw'] - hv_min) / amplitude
            r['Estabilidade'] = max(0, min(10, round(percentil_inv * 10)))
            del r['HV_raw']
