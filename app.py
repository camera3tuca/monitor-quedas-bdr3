...
def buscar_dados_fundamentalistas(ticker_bdr):
    ...
    # ------------------------------------------------------------------
    # TENTATIVA 1: Yahoo Finance — ticker US mapeado (empresa mãe)
    # ------------------------------------------------------------------
    try:
        tickers_tentar = [ticker_us]
        # Variantes comuns: BRK-B → BRK.B, etc.
        if '-' in ticker_us:
            tickers_tentar.append(ticker_us.replace('-', '.'))
        # Fallback: strip numérico do próprio BDR se o mapa não cobriu
        base_bdr = ticker_bdr.rstrip('0123456789')
        if base_bdr not in tickers_tentar:
            tickers_tentar.append(base_bdr)
    ...
    # ------------------------------------------------------------------
    # TENTATIVA 2: OpenBB / FMP — empresa mãe
    # ------------------------------------------------------------------
    try:
    ...
    # ------------------------------------------------------------------
    # TENTATIVA 3: BRAPI — BDR na B3 (último recurso)
    # ------------------------------------------------------------------
    try:
