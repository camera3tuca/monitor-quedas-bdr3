...
# =============================================================================
# CACHE PERSISTENTE — ticker US resolvido por nome (JSON local, TTL 30 dias)
# =============================================================================
...
def resolver_ticker_us(ticker_bdr: str, nome_empresa: str) -> str | None:
    """
    Resolve o ticker US correto via cache JSON + yf.Search() pelo nome.
    Corrige BDR_TO_US_MAP em memória ao encontrar um ticker válido.
    """
...
def buscar_dados_fundamentalistas(ticker_bdr):
    ...
    # ------------------------------------------------------------------
    # TENTATIVA 1: cache persistente + yf.Search() pelo nome da empresa mãe
    # ------------------------------------------------------------------
    # resolver_ticker_us() consulta o cache JSON local (TTL 30 dias).
    # Se não houver cache válido, faz yf.Search() pelo nome, valida e grava.
    # Também corrige BDR_TO_US_MAP em memória automaticamente.
    try:
...
def analisar_oportunidades(df_calc, mapa_nomes):
    ...
    for ticker in tickers:
        try:
            ...
            # === RANKING DE ESTABILIDADE — ATR% absoluto (padrão de mercado) ===
            # ATR% = Average True Range / preço médio × 100
            # Thresholds calibrados para BDRs em R$ (inclui efeito câmbio + spread B3)
            # ETF iShares ~0.5-1%  → 8-10  |  BDR estável ~1-1.5% → 6-8
            # BDR normal  ~2-3%    → 3-5   |  BDR agitada  >4%    → 0-2
            try:
                n_atr    = min(60, len(df_ticker))
                df_atr   = df_ticker.tail(n_atr)
                high_s   = df_atr['High']
                low_s    = df_atr['Low']
                close_s  = df_atr['Close']
                ...
                atr_pct = (tr.mean() / close_s.mean()) * 100

                # Tabela de conversão ATR% → ranking 0-10
                if   atr_pct <= 0.5: ranking_estab = 10
                elif atr_pct <= 0.8: ranking_estab = 9
                elif atr_pct <= 1.1: ranking_estab = 8
                elif atr_pct <= 1.4: ranking_estab = 7
                elif atr_pct <= 1.8: ranking_estab = 6
                elif atr_pct <= 2.3: ranking_estab = 5
                elif atr_pct <= 2.9: ranking_estab = 4
                elif atr_pct <= 3.6: ranking_estab = 3
                elif atr_pct <= 4.5: ranking_estab = 2
                elif atr_pct <= 5.5: ranking_estab = 1
                else:                ranking_estab = 0
            except Exception:
                ranking_estab = 5
            ...
            resultados.append({
                ...
                'Estabilidade': int(ranking_estab),
            })
        except: continue
...
def estilizar_estabilidade(val):
    """Paleta 0-10: 0=vermelho (agitada) → 10=verde (estável)"""
    paleta = {
    ...
    if filtrar_ema20 or filtrar_ema50 or filtrar_ema200 or ranking_min_liq > 0 or ranking_min_estab > 0:
        ...
        for opp in oportunidades:
            ...
            try:
                ...
                # Filtro de Estabilidade (exclui BDRs muito agitadas)
                if ranking_min_estab > 0 and passa_filtro:
                    if opp.get('Estabilidade', 10) < ranking_min_estab:
                        passa_filtro = False
    ...
    if not df_res.empty:
        ...
        evento = st.dataframe(
            df_res.style.map(estilizar_potencial, subset=['Potencial'])
                        ...
                        .map(estilizar_estabilidade, subset=['Estabilidade'])
            .format({
                'Preco': 'R$ {:.2f}',
                'Volume': '{:,.0f}',
                'Queda_Dia': '{:.2f}%',
            ...
            column_order=("Ticker", "Empresa", "Queda_Dia", "Liquidez", "Estabilidade", "Preco", "IS", "Volume", "Gap", "Potencial", "Score", "Sinais"),
            column_config={
                "Empresa": st.column_config.TextColumn("Empresa", width="medium"),
                "Liquidez": st.column_config.NumberColumn("💧 Liq.", width="small",
                    help="Ranking de Liquidez 0-10 (🔴 baixa → 🟢 alta)"),
                "Estabilidade": st.column_config.NumberColumn("🌊 Estab.", width="small",
                    help="Estabilidade 0-10 (🔴 agitada → 🟢 estável). Quanto maior, menos saltos."),
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col2:
                    ...
                    estab = row.get('Estabilidade', '-')
                    estab_emoji = '🟢' if isinstance(estab, int) and estab >= 7 else ('🟡' if isinstance(estab, int) and estab >= 4 else '🔴')
                    st.markdown(f"**🌊 Estabilidade:** {estab_emoji} {estab}/10")
