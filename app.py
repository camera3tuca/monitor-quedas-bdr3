...
# =============================================================================
# CACHE PERSISTENTE — ticker resolvido por nome (JSON local)
# =============================================================================
_CACHE_FILE = "bdr_ticker_cache.json"
_CACHE_TTL_DIAS = 30  # renova o mapeamento a cada 30 dias

def _carregar_cache() -> dict:
    """Lê o JSON de cache do disco. Retorna dict vazio se não existir."""
    try:
...
def _salvar_cache(cache: dict) -> None:
    """Persiste o dict de cache no disco."""
    try:
...
def _cache_valido(entrada: dict) -> bool:
    """Retorna True se a entrada do cache ainda está dentro do TTL."""
    try:
        salvo_em = datetime.fromisoformat(entrada.get('salvo_em', '2000-01-01'))
        return datetime.now() - salvo_em < timedelta(days=_CACHE_TTL_DIAS)
    except Exception:
        return False

def resolver_ticker_us(ticker_bdr: str, nome_empresa: str) -> str | None:
    """
    Resolve o ticker US correto para uma BDR usando cache persistente + yf.Search().
    Também corrige automaticamente o BDR_TO_US_MAP em memória.

    Fluxo:
      1. Verifica cache JSON — se válido, retorna imediatamente.
      2. Se não, faz yf.Search() pelo nome da empresa mãe.
      3. Valida o ticker encontrado com yf.Ticker().info.
      4. Salva no cache e corrige BDR_TO_US_MAP em memória.
    """
    cache = _carregar_cache()

    # 1. Cache hit
    if ticker_bdr in cache and _cache_valido(cache[ticker_bdr]):
        return cache[ticker_bdr].get('ticker_us')

    # 2. Busca pelo nome
    nome_limpo = nome_empresa
    for sufixo in [' ADR', ' ADS', ' Ordinary Shares', ' Class A', ' Class B',
                   ' Class C', ' A Shares', ' B Shares', ' Inc.', ' Corp.',
                   ' PLC', ' Ltd.', ' Limited', ' N.V.', ' S.A.', ' SE']:
    ...
    try:
        resultado_busca = yf.Search(nome_limpo, max_results=8)
        quotes = resultado_busca.quotes if hasattr(resultado_busca, 'quotes') else []

        bolsas_us = {'NMS', 'NYQ', 'NGM', 'NCM', 'ASE', 'PCX', 'BTS', 'NAS', 'NYSE', 'NASDAQ'}
        candidatos = [
            q.get('symbol', '')
            for q in quotes
            if q.get('quoteType') == 'EQUITY'
            and '.' not in q.get('symbol', '')
            and q.get('exchange', '') in bolsas_us
        ]

        # 3. Valida cada candidato
        for ticker_us in candidatos[:3]:
            try:
                info = yf.Ticker(ticker_us).info
                if info and len(info) >= 5 and any([
                    info.get('marketCap'),
                    info.get('trailingPE'),
                    info.get('forwardPE'),
                    info.get('revenueGrowth'),
                ]):
                    # 4a. Salva no cache JSON
                    cache[ticker_bdr] = {
                        'ticker_us': ticker_us,
                        'nome': nome_empresa,
                    ...
                    # 4b. Corrige BDR_TO_US_MAP em memória para esta sessão
                    BDR_TO_US_MAP[ticker_bdr] = ticker_us
...
def buscar_dados_fundamentalistas(ticker_bdr):
    ...
    # ------------------------------------------------------------------
    # TENTATIVA 1: cache persistente + yf.Search() pelo nome da empresa
    # ------------------------------------------------------------------
    # resolver_ticker_us() consulta o cache JSON local primeiro (TTL 30d).
    # Se não houver cache válido, faz yf.Search() pelo nome, valida o
    # ticker e grava no cache — corrigindo BDR_TO_US_MAP em memória.
    try:
        nome_empresa = NOMES_BDRS.get(ticker_bdr, '')
        if nome_empresa:
            ticker_resolvido = resolver_ticker_us(ticker_bdr, nome_empresa)
            if ticker_resolvido:
                info = yf.Ticker(ticker_resolvido).info
                resultado = _score_from_yf_info(
                    info,
                    f'Yahoo Finance — {ticker_resolvido} (via busca por nome)',
                    ticker_resolvido,
                )
                if resultado:
                    return resultado
...
def analisar_oportunidades(df_calc, mapa_nomes):
    ...
    for ticker in tickers:
        try:
            ...
            # === RANKING DE VOLATILIDADE (0-10, inverso — menor é melhor) ===
            try:
                n_vol = min(60, len(df_ticker))
                df_v = df_ticker.tail(n_vol)

                # 1. ATR % — amplitude média True Range em % do preço
                high_s = df_v['High']
                low_s  = df_v['Low']
                close_s = df_v['Close']
                ...
                atr_pct = (tr.mean() / close_s.mean()) * 100  # ATR como % do preço

                # 2. Desvio padrão dos retornos diários (%)
                retornos = close_s.pct_change().dropna() * 100
                std_ret  = retornos.std()

                # 3. Frequência de saltos diários > 3%
                saltos_grandes = (retornos.abs() > 3).sum() / len(retornos) * 100

                # Penalidades acumuladas (maior penalidade = mais volátil = nota menor)
                penalidade = 0

                # ATR % (até 40 pts de penalidade)
                if   atr_pct > 6:  penalidade += 40
                elif atr_pct > 4:  penalidade += 30
                elif atr_pct > 3:  penalidade += 22
                elif atr_pct > 2:  penalidade += 14
                elif atr_pct > 1.5: penalidade += 8
                elif atr_pct > 1:  penalidade += 4
                # else: nenhuma penalidade

                # Desvio padrão dos retornos (até 35 pts)
                if   std_ret > 4:  penalidade += 35
                elif std_ret > 3:  penalidade += 25
                elif std_ret > 2:  penalidade += 16
                elif std_ret > 1.5: penalidade += 9
                elif std_ret > 1:  penalidade += 4
                # else: nenhuma penalidade

                # Frequência de saltos > 3% (até 25 pts)
                if   saltos_grandes > 30: penalidade += 25
                elif saltos_grandes > 20: penalidade += 18
                elif saltos_grandes > 10: penalidade += 10
                elif saltos_grandes > 5:  penalidade += 5
                # else: nenhuma penalidade

                # Converte penalidade (0-100) em ranking (0-10), invertido
                ranking_vol = max(0, min(10, round((100 - penalidade) / 10)))
            except Exception:
                ranking_vol = 5  # neutro se falhar
...
def estilizar_volatilidade(val):
    """Degradê verde→amarelo→vermelho para ranking 0-10 (inversão: 10=baixa vol=verde)"""
    paleta = {
        0:  ('#7f0000', 'white'),   # altíssima volatilidade
        1:  ('#c62828', 'white'),
        2:  ('#ef5350', 'white'),
        3:  ('#ff7043', 'white'),
        4:  ('#ffa726', 'black'),
        ...
        8:  ('#66bb6a', 'black'),
        9:  ('#2e7d32', 'white'),
        10: ('#1b5e20', 'white'),   # baixíssima volatilidade
    }
    try:
        v = int(val)
    except Exception:
...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    # Sliders em duas colunas para economizar espaço
    col_sl1, col_sl2 = st.columns(2)
    with col_sl1:
        st.markdown("**💧 Liq. mínima:**")
        ranking_min_liq = st.slider(
            "0=livre | 10=máximo",
            min_value=0, max_value=10, value=0, step=1,
            key="slider_liq",
            help="Ranking de liquidez 0-10. Quanto maior, menor o risco de gaps e volume baixo."
        )
    with col_sl2:
        st.markdown("**🌊 Estabilidade mínima:**")
        ranking_min_estab = st.slider(
            "0=livre | 10=máximo",
            min_value=0, max_value=10, value=0, step=1,
            key="slider_vol",
            help="Ranking de estabilidade 0-10. Aumente para excluir ações muito agitadas/saltitantes."
        )

    # Aplicar filtros se algum selecionado
    if filtrar_ema20 or filtrar_ema50 or filtrar_ema200 or ranking_min_liq > 0 or ranking_min_estab > 0:
        df_res_filtrado = []
        ...
        for opp in oportunidades:
            ...
            try:
                ...
                # Filtro de Volatilidade: exclui BDRs com ranking ABAIXO do máximo permitido
                # Ex: slider em 6 → exclui BDRs com Volatilidade 0-5 (muito agitadas)
                if ranking_min_estab > 0 and passa_filtro:
                    if opp.get('Volatilidade', 10) < ranking_min_estab:
                        passa_filtro = False
    ...
    if not df_res.empty:
        ...
        .map(estilizar_volatilidade, subset=['Volatilidade'])
            .format({
                'Preco': 'R$ {:.2f}',
                'Volume': '{:,.0f}',
                'Queda_Dia': '{:.2f}%',
            ...
            column_order=("Ticker", "Empresa", "Liquidez", "Volatilidade", "Preco", "Queda_Dia", "IS", "Volume", "Gap", "Potencial", "Score", "Sinais"),
            column_config={
                "Empresa": st.column_config.TextColumn("Empresa", width="medium"),
                "Liquidez": st.column_config.NumberColumn("💧 Liq.", width="small",
                    help="Ranking de Liquidez 0-10 (🔴 baixa → 🟢 alta)"),
                "Volatilidade": st.column_config.NumberColumn("🌊 Estab.", width="small",
                    help="Estabilidade 0-10 (🔴 muito agitada → 🟢 estável). Quanto maior, menos saltos."),
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col2:
                    ...
                    vol_rank = row.get('Volatilidade', '-')
                    vol_emoji = "🟢" if isinstance(vol_rank, int) and vol_rank >= 7 else ("🟡" if isinstance(vol_rank, int) and vol_rank >= 4 else "🔴")
                    st.markdown(f"**🌊 Volatilidade:** {vol_emoji} {vol_rank}/10")
