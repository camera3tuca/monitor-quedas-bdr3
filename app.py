...
def buscar_dados_fundamentalistas(ticker_bdr):
    ...
    2. Se falhar, tenta OpenBB (empresa mãe)
    3. Se falhar, tenta BRAPI (BDR na B3)
    """
    ...
    try:
        ...
        if info_obb:
            score, detalhes = calcular_score_fundamentalista(info_obb)

            return {
                'fonte': 'OpenBB / FMP (Empresa Mãe)',
                'ticker_fonte': ticker_us,
                'score': score,
                'detalhes': detalhes,
                'pe_ratio': info_obb.get('trailingPE') or info_obb.get('forwardPE'),
                'dividend_yield': info_obb.get('dividendYield'),
                'market_cap': info_obb.get('marketCap'),
                'revenue_growth': info_obb.get('revenueGrowth'),
                'recomendacao': info_obb.get('recommendationKey'),
                'setor': info_obb.get('sector', 'N/A'),
            }
    except Exception:
        pass
