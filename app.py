...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            # Tenta obter nome da empresa e setor a partir dos dados fundamentalistas (se disponíveis)
            ...
            # Linha informativa com empresa e setor
            ...
            with info_cols[1]:
                btn_atualizar = st.button("🔄 Atualizar notícias", key=f"btn_news_{ticker}")

            with st.spinner("Buscando notícias nas fontes financeiras..."):
                noticias_lista = buscar_todas_noticias(
                    ticker_bdr=ticker,
                    ticker_us=ticker_us_news,
                    empresa_nome=empresa_nome_news,
                    setor=setor_news
                )

            if noticias_lista:
                # Agrupa por fonte para exibir resumo
                fontes_encontradas = list(dict.fromkeys(n['fonte'] for n in noticias_lista))
                st.caption(
                    f"✅ {len(noticias_lista)} notícias encontradas | "
                    f"Fontes: {', '.join(fontes_encontradas)}"
                )

                # Exibe os cards em 2 colunas
                col_n1, col_n2 = st.columns(2)
                metade = (len(noticias_lista) + 1) // 2
                with col_n1:
                    for i, noticia in enumerate(noticias_lista[:metade]):
                        st.markdown(renderizar_card_noticia(noticia, i), unsafe_allow_html=True)
                with col_n2:
                    for i, noticia in enumerate(noticias_lista[metade:]):
                        st.markdown(renderizar_card_noticia(noticia, i + metade), unsafe_allow_html=True)
                ...
                with link_cols[0]:
                    st.markdown(
                        f"[📊 Yahoo Finance](https://finance.yahoo.com/quote/{ticker_us_news}/news/)",
                        unsafe_allow_html=False
                    )
                with link_cols[1]:
                    st.markdown(
                        f"[📈 Seeking Alpha](https://seekingalpha.com/symbol/{ticker_us_news}/news)",
                        unsafe_allow_html=False
                    )
                with link_cols[2]:
                    st.markdown(
                        f"[🔍 Finviz](https://finviz.com/quote.ashx?t={ticker_us_news})",
                        unsafe_allow_html=False
                    )
                with link_cols[3]:
                    st.markdown(
                        f"[🧙 GuruFocus](https://www.gurufocus.com/news/{ticker_us_news})",
                        unsafe_allow_html=False
                    )
            else:
                st.warning(
                    f"⚠️ Não foi possível buscar notícias para **{empresa_nome_news}** (`{ticker_us_news}`) "
                    f"no momento. Tente acessar diretamente:"
                )
                link_cols2 = st.columns(4)
                with link_cols2[0]:
...
def limpar_html(texto):
    """Remove tags HTML e decodifica entidades HTML de um texto."""
    if not texto:
        return ""
...
def buscar_noticias_yahoo_rss(ticker_us, empresa_nome, max_noticias=6):
    """Busca notícias via RSS do Yahoo Finance para o ticker US."""
    noticias = []
    headers = {
        'User-Agent': (
    ...
    for url in urls_rss:
        try:
            ...
            for item in items[:max_noticias]:
                ...
                fonte   = limpar_html(item.findtext(
                    '{http://purl.org/dc/elements/1.1/}creator', 'Yahoo Finance'
                ))
                if not titulo:
                    continue
                # Formata data
                ...
                noticias.append({
                    ...
                    'link': link,
                    'data': data_fmt,
                    'descricao': desc[:250] + ('...' if len(desc) > 250 else ''),
...
def buscar_noticias_finviz(ticker_us, max_noticias=6):
    """Busca notícias na página do Finviz para o ticker US."""
    noticias = []
    headers = {
        'User-Agent': (
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
    ...
    try:
        url = f"https://finviz.com/quote.ashx?t={ticker_us}"
        resp = requests.get(url, headers=headers, timeout=10)
        if resp.status_code != 200:
            return []
        # Extrai tabela de notícias via regex simples (sem BeautifulSoup)
        padrao = r'<a[^>]+href="(https?://[^"]+)"[^>]*class="[^"]*tab-link[^"]*"[^>]*>([^<]+)</a>'
        matches = re.findall(padrao, resp.text)
        # Data: linha anterior ao link costuma ter a data no Finviz
        ...
        for i, (link, titulo) in enumerate(matches[:max_noticias]):
            ...
            if not titulo or len(titulo) < 10:
                continue
            data_fmt = datas[i] if i < len(datas) else ''
            noticias.append({
                'titulo': titulo,
                'link': link,
                'data': data_fmt,
...
def buscar_noticias_gurufocus_rss(ticker_us, max_noticias=5):
    """Busca notícias via RSS do GuruFocus."""
    noticias = []
    headers = {
        'User-Agent': (
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
    ...
    try:
        url = f"https://www.gurufocus.com/news/rss/{ticker_us}"
        resp = requests.get(url, headers=headers, timeout=8)
        if resp.status_code != 200:
            return []
        root = ET.fromstring(resp.content)
...
def buscar_noticias_seekingalpha_rss(ticker_us, max_noticias=5):
    """Busca notícias via RSS do Seeking Alpha."""
    noticias = []
    headers = {
        'User-Agent': (
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
...
def buscar_todas_noticias(ticker_bdr, ticker_us, empresa_nome, setor):
    """
    Agrega notícias de múltiplas fontes para o ticker e empresa-mãe.
    Retorna lista de dicts ordenada por fonte.
    """
    todas = []

    # 1. Yahoo Finance RSS (mais confiável)
    todas += buscar_noticias_yahoo_rss(ticker_us, empresa_nome, max_noticias=6)

    # 2. GuruFocus RSS
    if len(todas) < 4:
        todas += buscar_noticias_gurufocus_rss(ticker_us, max_noticias=5)

    # 3. Seeking Alpha RSS
    if len(todas) < 6:
        todas += buscar_noticias_seekingalpha_rss(ticker_us, max_noticias=5)

    # 4. Finviz (scraping)
    if len(todas) < 4:
        todas += buscar_noticias_finviz(ticker_us, max_noticias=6)
...
def renderizar_card_noticia(noticia, idx):
    """Renderiza um card de notícia em HTML."""
    ...
    bg, cor_fonte, badge_bg = fonte_cores.get(fonte, ('#f8fafc', '#475569', '#e2e8f0'))

    desc_html = f"<p style='margin: 0.4rem 0 0 0; font-size: 0.82rem; color: #64748b; line-height: 1.4;'>{desc}</p>" if desc else ""
