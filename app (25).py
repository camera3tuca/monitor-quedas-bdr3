...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    # Slider de volatilidade
    st.markdown("**🌊 Volatilidade máxima:**")
    ranking_max_vol = st.slider(
        "10 = sem filtro  |  0 = mínima exigência",
        min_value=0, max_value=10, value=10, step=1,
        key="slider_vol",
        help="Filtra BDRs pelo ranking de volatilidade 0-10 (inverso). Valores altos = ação mais estável."
    )

    # Aplicar filtros se algum selecionado
    if filtrar_ema20 or filtrar_ema50 or filtrar_ema200 or ranking_min_liq > 0 or ranking_max_vol < 10:
        df_res_filtrado = []
        ...
        for opp in oportunidades:
            ...
            try:
                ...
                # Filtro de Volatilidade (ranking_max_vol < 10 ativa o filtro)
                if ranking_max_vol < 10 and passa_filtro:
                    if opp.get('Volatilidade', 10) < ranking_max_vol:
                        passa_filtro = False
    ...
    if not df_res.empty:
        ...
        .map(estilizar_volatilidade, subset=['Volatilidade'])
            ...
            column_config={
                ...
                "Volatilidade": st.column_config.NumberColumn("🌊 Vol.", width="small",
                    help="Ranking de Volatilidade 0-10 (🔴 alta → 🟢 baixa). Quanto maior, mais estável."),
